...
### Dataset Creation Batch Script

To download Mega Man Maker levels and apply deterministic captions in one go, simply call:
```
cd MM_batch
MMLV-data.bat [target] [no_traversability_filter]
```
*both arguments are optional; target defaults to 5,000, and without the no_traversability_filter argument levels
are filtered with an Astar traversability measure*


If you'd like to add additional captions to the dataset afterwards with LLMs (discussed later), call:
```
REM python llm_ascii_to_caption.py --levels datasets\MMLV_LevelsAndCaptions-multi.json --game mmlv --output datasets\MMLV_LevelsAndCaptions-multi.json --llm [ollama, claude, openai] --model [model_name] --num_captions [num_captions]
```
This will add an additional json field "{model_name}_captions" that contains a list of {num_captions} captions generated by
your chosen LLM to each sample in the dataset. 


Lastly, to split your dataset in preparation for training, call:
```
...
```bash
python ascii_data_browser.py MMLV_conditional_regular0-samples\all_levels.json Game_MMLV\MMLV.json
```
...
python MM_create_ascii_captions.py --dataset datasets\MM_Levels-simple.json --tileset Game_MM/MM-Simple-tileset.json --output datasets\MM_LevelsAndCaptions-simple-regular.json
...
Browse:
```
python ascii_data_browser.py MM_conditional_simple_regular0-samples\all_levels.json Game_MM/MM-Simple-tileset.json
```
...
```
python create_tile_level_json_data.py --tileset Game_MM/MM-Simple-tileset.json --levels ..\TheVGLC\MegaMan\Enhanced --output datasets\MM_3x3_Tiles-simple.json --tile_size 3 --char_map datasets\MM-VGLC-to-simple.json
```
