...
python train_diffusion.py --save_image_epochs 10 --augment --text_conditional --output_dir "%DIFF_OUTPUT%" --num_epochs 100 --json datasets\%DATA%_LevelsAndCaptions-regular-train.json --val_json datasets\%DATA%_LevelsAndCaptions-regular-validate.json --pkl datasets\%LABEL%_Tokenizer-regular.pkl --mlm_model_dir %MLM_OUTPUT% --plot_validation_caption_score --seed %SEED% --batch_size 16 
