...
def create_plot(experiment_data, style_indices, error_type=None, confidence=0.95, legend_loc='lower right', ymin=-0.2, ymax=1.0, title=None):
    ...
    if legend_loc.startswith('bbox_to_anchor'):
        ...
        legend = plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', prop={'size': 22}, handlelength=2.5, handletextpad=1.5, fontsize=22)
    else:
        legend = plt.legend(loc=legend_loc, prop={'size': 16}, handlelength=2.5, handletextpad=1.5, ncol=2, fontsize=22)
