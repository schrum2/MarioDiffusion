...
Instead of the deterministic captions from `MM_create_ascii_captions.py`, you can choose to caption levels or scenes with an LLM. This process produces multiple diverse natural-language captions per sample, grounded on pre-computed structural metadata about the scene. Here is a command that uses Ollama with the local LLM `qwen3.5:9b` to create captions for the `MM-Simple` dataset.
```
python llm_ascii_to_caption.py --levels Game_MM\DATA\MM-Simple_LevelsAndCaptions-regular.json --game MM-Simple --llm ollama --model qwen3.5:9b --output Game_MM\DATA\MM-Simple_LLM_LevelsAndCaptions-regular.json --num_captions 5
```
The process takes a while to run, in part because each sample gets `--num_captions` number of captions, and there is error checking with retry attempts if the LLM fails to provide the correct number of captions or the results have other structural issues. If you only want to caption a few samples for the sake of testing, then the `--limit` parameter can be used. Note that the output from the command above will be a copy of the `--levels` json input, but with the additional captions added. As a result, it is possible to add captions generated by various different LLMs to the same data through repeated use of the `llm_ascii_to_caption.py` script. However, it is also possible to generate a brand new json dataset from the original VGLC levels, though this approach assigns a single caption to each complete level (each txt file from the VGLC) rather than to individual scenes: 
```
python llm_ascii_to_caption.py --levels ..\TheVGLC\MegaMan\Enhanced --game MM-Full --llm ollama --model gemma4:26b --output Game_MM\DATA\MM-Full_LLMLevel_LevelsAndCaptions-regular.json --num_captions 5
```
This example also switches the game to `MM-Full` and changes the LLM to `gemma4:26b` simply to show the variety of options that are possible.
